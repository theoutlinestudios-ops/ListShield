...
export async function GET(request: Request) {
  ...
  const result = await fetch('https://api.etsy.com/v3/public/oauth/token', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ grant_type: 'authorization_code', client_id: process.env.ETSY_API_KEY || '', redirect_uri: `${url.origin}/api/etsy/oauth/callback`, code: url.searchParams.get('code') || '', code_verifier: verifier }) })
