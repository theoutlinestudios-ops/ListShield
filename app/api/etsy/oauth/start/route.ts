...
export async function GET(request: Request) {
  ...
  const response = NextResponse.redirect(`https://www.etsy.com/oauth/connect?response_type=code&client_id=${encodeURIComponent(process.env.ETSY_API_KEY || '')}&redirect_uri=${encodeURIComponent(`${process.env.BETTER_AUTH_URL || new URL(request.url).origin}/api/etsy/oauth/callback`)}&scope=listings_r%20shops_r&state=${state}&code_challenge=${challenge}&code_challenge_method=S256`)
